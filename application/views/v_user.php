    ...
    <div class="header-body">
      <div class="row align-items-center py-4">
        ...
        <div class="col-lg-6 col-5 text-right">
          <a href="<?php echo base_url('User/tambah') ?>" class="btn btn-sm btn-neutral">New</a>
          ...
          <table class="table table-flush" id="datatable-basic">
            <thead class="thead-light">
              <tr>
                ...
                <th>Nama</th>
                <th>Telp</th>
                <th>Tanggal pembaruan/daftar</th>
                <th>Status</th>
                <th>Jatuh Tempo</th>
                <th>Action</th>
              </tr>
            ...
            <tbody>
              <?php $no=1; foreach ($user->result() as $var): ?>
              <tr>
                <td><?php echo $no++ ?></td>
                <td><a href="" data-toggle="modal" data-target="#modal-default<?php echo $var->kd_user ?>"><?php echo $var->nama_depan.' '.$var->nama_belakang ?></a></td>
                <td>+62<?php echo $var->no_telp ?></td>
                <td><?php echo $var->tgl_daftar ?></td>
                <td><?php if ($var->status_user == 0): ?>
                <a href="" class="btn btn-danger btn-sm">Tidak Aktif</a>
                <?php else: ?>
                  <a href="" class="btn btn-success btn-sm">Aktif</a>
                  <?php endif ?></td>
                  <td><?php echo $var->tgl_jatuh_tempo  ?></td>
                  <td>
                    <a href="<?php echo base_url('User/updateview/'.$var->kd_user) ?>" class="btn-sm btn-warning" title="Update"><i class="ni ni-ruler-pencil "></i></a>
                    <?php if (!$var->status_user == 1): ?>
                      <a href="<?php echo base_url('User/active/'.$var->kd_user) ?>" title="Aktifkan" onclick="javascript: return confirm('Anda Yakin Akan ingin aktifkan User ?')" class="btn-sm btn-success" ><i class="fa fa-check"></i></a>
                      <?php else: ?>
                        <a href="<?php echo base_url('User/hapus/'.$var->kd_user) ?>" title="Nonaktifkan" onclick="javascript: return confirm('Anda Yakin Akan ingin Nonaktifkan User ?')" class="btn-sm btn-danger" ><i class="fa fa-window-close"></i></a>
                      <?php endif ?>

                    </td>
                    <div class="modal fade" id="modal-default<?php echo $var->kd_user ?>" tabindex="-1" role="dialog" aria-labelledby="modal-default" style="display: none;" aria-hidden="true">
